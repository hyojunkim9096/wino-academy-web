...
import { getTeacherEvents, getClassEvents } from '@/features/course/api/timetableApi.js';
        ...
        <div className="aa-page academy-page">
            <div className="aa-container">
                <div className="aa-toolbar aa-topbar">
                    ...
                    <select className="aa-select" value={semesterId??''} onChange={e=>setSemesterId(e.target.value?Number(e.target.value):null)}>
                        <option value="">학기 전체</option>
                        {semesters.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}
                    </select>
                    {mode==='teacher' && (
                        <div className="grow" style={{maxWidth:420}}>
                            <HomeroomPicker value={teacherId} onChange={setTeacherId} workLocation={work} allowAllLocations={false} placeholder="교사 선택" />
                        </div>
                    )}
                    {mode==='class' && (
                        <select className="aa-select" value={classId??''} onChange={e=>setClassId(e.target.value?Number(e.target.value):null)}>
