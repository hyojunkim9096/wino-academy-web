...
// 하위 탭 컴포넌트
import StudentEnrollments from '@/features/admin/components/student/StudentEnrollments';
import StudentConsults    from '@/features/admin/components/student/StudentConsults';
...
export default function AdminStudentPage(){
        ...
        (async ()=>{
            try{
                const [loc, stg, rel, sts, notice, enrollStat] = await Promise.all([
                    getCodes(CODE_LOC), getCodes(CODE_STAGE),
                    getCodes(CODE_FAMILY_REL), getCodes(CODE_STUDENT_STATUS),
    ...
    const addQuickMemo = async (contentText)=>{
        ...
        try{
            const { addStudentMemo } = await import('@/api/studentMemoApi');
        ...
        <section className="aa-page academy-page student-page">
                        ...
                        <div className="space-y-2 mb-3">
                            <div className="filter-row">
                                ...
                                <select className="aa-select" value={workLoc} onChange={e=>setWorkLoc(e.target.value)}>
                                    ...
                                    {sorted(locCodes).map(l=>(<option key={l.code} value={l.code}>{l.name} ({l.code})</option>))}
                                </select>
                            </div>
                            <div className="filter-row">
                                ...
                                <select className="aa-select" value={stage} onChange={e=>setStage(e.target.value)}>
                                    <option value="">전체</option>
                                    {sorted(stgCodes).map(s=>(<option key={s.code} value={s.code}>{s.name} ({s.code})</option>))}
                                </select>
                            </div>
                            <div>
                                <input className="aa-input w-full" placeholder="이름/연락처/이메일/아이디" value={keyword} onChange={e=>setKeyword(e.target.value)} />
                            </div>
                        </div>
                                        ...
                                        <button key={row.id} type="button"
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="truncate">
                                                    {row.name}
                                                    <span className="text-slate-400 font-normal"> ({stgName(row.schoolStage)} · {locName(row.workLocationCode)})</span>
                                                </div>
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div className="text-sm text-slate-400">아이디와 비밀번호를 함께 입력하면, 생성 직후 계정을 세팅합니다.</div>
                                        </div>
                                    </div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="grid md:grid-cols-3 gap-3">
                                        <Field label="아이디(로그인 ID)">
                                            <input className="aa-input" value={newForm.loginId}
                                                   onChange={e=>setNewForm(f=>({...f, loginId: e.target.value}))}
                                                   placeholder="예: student01" />
                                        </Field>
                                        <Field label="비밀번호">
                                            <input type="password" className="aa-input" value={newForm.password}
                                    ...
                                    <div className="grid md:grid-cols-3 gap-3">
                                        <Field label="이름">
                                            <input className="aa-input" value={newForm.name}
                                    ...
                                    <div className="grid md:grid-cols-3 gap-3">
                                        <Field label="학부">
                                            <select className="aa-select" value={newForm.schoolStage}
                                                    onChange={e=>{
                                                        const v = e.target.value;
                                                        setNewForm(f=>({...f, schoolStage:v, grade:'', schoolId:null, schoolName:''}));
                                                    }}>
                                                <option value="">선택</option>
                                                {sorted(stgCodes).map(s=>(<option key={s.code} value={s.code}>{s.name} ({s.code})</option>))}
                                            </select>
                                        ...
                                        <Field label="학교">
                                            <div className="flex gap-2">
                                                <input className="aa-input flex-1" placeholder="선택된 학교" value={newForm.schoolName||''} readOnly />
                                                <button className="aa-btn" onClick={()=>{
                                                    if(!newForm.schoolStage) return safeInfo('안내','먼저 학부를 선택하세요.');
                                                    setSchoolQuery(''); setSchoolResults([]); setSchoolPickOpen(true);
                                                }}>학교 선택</button>
                                            </div>
                                        </Field>
                                        <Field label="학년">
                                            <select className="aa-select" value={newForm.grade||''}
                                    ...
                                    <div className="grid md:grid-cols-2 gap-3">
                                        <Field label="연락처">
                                            <input className="aa-input" value={newForm.phone||''}
                                                   onChange={e=>setNewForm(f=>({...f, phone:e.target.value}))}/>
                                        </Field>
                                        <Field label="이메일">
                                            <input className="aa-input" value={newForm.email||''}
                                    ...
                                    {appEnabled && (
                                        <div className="grid md:grid-cols-3 gap-3">
                                            <Field label="Push User Key (선택)">
                                                <input className="aa-input" value={newForm.pushUserKey||''}
                                    ...
                                    <div className="grid md:grid-cols-12 gap-3">
                                        ...
                                        <div className="md:col-span-5">
                                            <Field label="주소">
                                                <input className="aa-input" value={newForm.address || ''} readOnly />
                                            </Field>
                                        </div>
                                        <div className="md:col-span-4">
                                            <Field label="상세주소">
                                                <input
                                                    ...
                                                    onChange={e => setNewForm(f => ({ ...f, detailAddress: e.target.value }))}
                                                />
                                            </Field>
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        <Field label="비고(요약메모)">
                      <textarea className="aa-textarea" rows={4} value={newForm.memo||''}
                                    ...
                                    <div className="space-y-4">
                                            ...
                                            <div className="flex items-start gap-4">
                                                <div>
                                                    {!editing ? (
                                                        ...
                                                        <Avatar size={80} src={photoSrc} name={detail.name} version={imgVersion}/>
                                                    )}
                                                </div>
                                                <div>
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div className="grid md:grid-cols-3 gap-3">
                                                <Field label="이름">
                                                    {!editing ? <RO>{detail.name}</RO> : (
                                                        <input className="aa-input" value={edit.name} onChange={e=>setEdit(f=>({...f,name:e.target.value}))}/>
                                                    )}
                                                </Field>
                                                <Field label="생년월일">
                                                    {!editing ? <RO>{detail.birthdate || '-'}</RO> : (
                                            ...
                                            >
                                                ...
                                                <div className="grid md:grid-cols-3 gap-3">
                                                    <Field label="학부">
                                                        {!editing ? <RO>{stgName(detail.schoolStage)}</RO> : (
                                                            <select className="aa-select" value={edit.schoolStage}
                                                                    onChange={e=>{
                                                                        const v = e.target.value;
                                                                        setEdit(f=>({...f, schoolStage:v, grade:'', schoolId:null, schoolName:''}));
                                                                    }}>
                                                                {sorted(stgCodes).map(s=>(<option key={s.code} value={s.code}>{s.name} ({s.code})</option>))}
                                                            </select>
                                                        )}
                                                    ...
                                                    <Field label="학교">
                                                        {!editing ? (
                                                            ...
                                                            <div className="flex gap-2">
                                                                <input className="aa-input flex-1" value={edit.schoolName||''} readOnly placeholder="선택된 학교"/>
                                                                <button className="aa-btn" onClick={()=>{
                                                                    if(!edit.schoolStage) return safeInfo('안내','먼저 학부를 선택하세요.');
                                                                    setSchoolQuery(''); setSchoolResults([]); setSchoolPickOpen(true);
                                                                }}>학교 선택</button>
                                                            </div>
                                                        )}
                                                    ...
                                                    <Field label="학년">
                                                        {!editing ? <RO>{detail.gradeLabel ? `${detail.gradeLabel}학년` : '-'}</RO> : (
                                                            <select className="aa-select" value={edit.grade||''}
                                                ...
                                                <div className="grid md:grid-cols-2 gap-3">
                                                    <Field label="연락처">
                                                        {!editing ? <RO>{detail.phone || '-'}</RO> : (
                                                            <input className="aa-input" value={edit.phone||''} onChange={e=>setEdit(f=>({...f,phone:e.target.value}))}/>
                                                        )}
                                                    </Field>
                                                    <Field label="이메일">
                                                        {!editing ? <RO>{detail.email || '-'}</RO> : (
                                                            <input className="aa-input" value={edit.email||''} onChange={e=>setEdit(f=>({...f,email:e.target.value}))}/>
                                                        )}
                                                    </Field>
                                                </div>
                                        ...
                                        {!editing && (
                                            <div className="border-t border-slate-700 pt-3">
                                                <div className="aa-seg mb-3">
                                                    <button className={tab==='ENROLL'?'active':''}  onClick={()=>setTab('ENROLL')}>반 배정</button>
                                                    <button className={tab==='TUITION'?'active':''} onClick={()=>setTab('TUITION')}>수강료</button>
            ...
            {schoolPickOpen && (
                <Modal title="학교 검색" onClose={()=>setSchoolPickOpen(false)} size="2xl">
                    <div className="space-y-3">
                        ...
                        <div className="flex gap-2">
                            <input className="aa-input flex-1" placeholder="학교명/주소"
                                   value={schoolQuery} onChange={e=>setSchoolQuery(e.target.value)} />
                            <button className="aa-btn" disabled={schoolLoading}
                        ...
                        <div className="aa-table-wrap max-h-[60vh] overflow-auto">
                            <table className="aa-table">
                                ...
                                {schoolResults.map(s=>(
                                    <tr key={s.id}>
                                        <td>{s.name}</td>
                                        <td className="aa-ellipsis">{s.address||s.detailAddress||'-'}</td>
                                        <td>
                                            <button className="aa-btn aa-btn-primary aa-btn-sm" onClick={()=>{
                                                if (creating){
            ...
            <PwModal
                ...
                onChangePassword={async (pw)=>{
                    if (!selectedId) return;
                    if ((pw||'').trim().length < 6) return safeInfo('안내','비밀번호는 6자리 이상이어야 합니다.');
                    try{
                        await changeStudentPassword(selectedId, pw);
                        await safeOk('성공','비밀번호가 변경되었습니다.');
                    }catch(e){
                        const st = e?.response?.status;
                        if (st===400) safeError('안내','비밀번호는 6자리 이상 등록해야합니다.');
                        else safeError('오류', e?.response?.data?.message || '비밀번호 변경 실패');
                    }
                }}
            />
            ...
            <div className="space-y-3">
                <div>
                    ...
                    <input type="password" className="aa-input aa-input-contrast w-full"
                           placeholder="새 비밀번호" value={pwForm.pw}
                           onChange={(e)=>setPwForm(f=>({...f, pw:e.target.value}))}/>
                </div>
                <div>
                    <div className="text-sm mb-1">새 비밀번호 확인</div>
                    <input type="password" className="aa-input aa-input-contrast w-full"
                           placeholder="새 비밀번호 확인" value={pwForm.pw2}
                           onChange={(e)=>setPwForm(f=>({...f, pw2:e.target.value}))}/>
                </div>
                <div className="flex justify-end gap-2 pt-2">
                    <button className="aa-btn" onClick={()=>setPwOpen(false)} disabled={saving}>취소</button>
...
function QuickMemoModal({open, onClose, onSubmit}){
    ...
    const [saving, setSaving] = useState(false);
    useEffect(()=>{ if(open){ setText(''); setSaving(false);} },[open]);
    if(!open) return null;
    return (
        <Modal title="메모 남기기" onClose={onClose}>
            <div className="space-y-3">
        <textarea className="aa-textarea w-full" rows={5}
                  placeholder="히스토리에 남길 메모를 입력하세요."
                  value={text} onChange={e=>setText(e.target.value)} />
                <div className="flex justify-end gap-2">
                    <button className="aa-btn" onClick={onClose} disabled={saving}>취소</button>
                    <button className="aa-btn aa-btn-primary" disabled={saving || !text.trim()} onClick={async ()=>{
                        try{
                            setSaving(true);
